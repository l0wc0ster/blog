...
To study the network, you can use the wireshark utility, we will find out which devices are working on the network, the names and software versions of routers, hostnames, name resolution protocols to IP addresses (LLMNR , NBT-NS), service protocols, etc. To complete the picture, we can do:
...
If the network has LLMNR / NBTNS protocols :
...
```
or you can try assigning IPv 6 addresses yourself:
...
The account mask can be found using document metadata, can be searched on the web using queries **site: jaki.local filetype: doc / pdf / docx / xlsx**, or use specialized software such as Foca.
...
Go to SMB shares, get information from RPC, use MITM and infect name resolution protocols - figure out the account mask. If, as part of a MITM attack, you received netTNTLMv1 with the --lm key for the responder - use https://crack.sh/get-cracking/ to restore pure NTLM for the user by sending NTHASH:responder_hash. If you got netNTLMv2, the only option is to brute-force:
...
here you need to understand that you can be caught by network anomaly search tools or NGFW.
...
Don't forget the techniques for getting a domain administrator using [AD CS](https://research.ifcr.dk/certipy-2-0-bloodhound-new-escalations-shadow-credentials-golden-certificates-and-more-34d1c26f0dc6)


The most common privilege escalation techniques are ESC 1 and ESC 8:
...
```
Make SMB Beacon - random name
than inject PID x64 PIPENAME_BEACON
and then inject several times with chain into different process ( svchost , systemtray , etc ) to bypass AV
```
...
than edit your tds.py find both lines defining the ctx: (and all such scripts)
...
Change line in GetUsersPNs.py to say this: (and all scripts)
