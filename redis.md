...
Imagine the situation when you need to get the RCE to the server from Linux, using the Redis database. At the same time, you have a password for the database, or you can go to the base anonymously. In this case, you can try to record the public key to the authorized_keys file along the path /var/lib/redis/.ssh.
